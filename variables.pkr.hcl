...
variable "vm_machine" {
  ...
  validation {
    ...
    error_message = "The vm_machine value must be 'pc' or 'q35'."
  }
}

variable "vm_vga" {
  type        = list(map(string))
  description = "The graphics adapter to use"
  default     = [
    {
      "type": "std",
      "memory": 32
...
variable "vm_serials" {
  type        = list(string)
  description = "A list (max 4 elements) of serial ports attached to the virtual machine. It may pass through a host serial device /dev/ttyS0 or create unix socket on the host socket. Each element can be socket or responding to pattern /dev/.+"
  default     = []
}
...
variable "vm_ssd" {
  type        = bool
  description = "Drive will be presented to the guest as solid-state drive rather than a rotational disk. This cannot work with virtio disks."
  default     = false
}

variable "vm_io_thread" {
  type        = bool
  description = "Create one I/O thread per storage controller, rather than a single thread for all I/O. This can increase performance when multiple disks are used. Requires virtio-scsi-single controller and a scsi or virtio disk."
  default     = false
}
...
variable "vm_firewall" {
  type        = bool
  description = "If the adapter should have the firewall enabled."
  default     = false
}
